...
export const publishToAccFolderSchema = z
  .object({
    project_id: z
      ...
      .optional()
      .describe(
        "ACC/BIM360 project id, e.g. 'b.ec81e371-8332-4663-a0fe-1a9579465bd1' (the 'b.' prefix is expected). " +
    ...
    project_name: z
      ...
      .optional()
      .describe(
        "Project name to resolve instead of project_id, e.g. 'Demoland Building 1'. The tool lists projects " +
    ...
    folder_id: z
      ...
      .optional()
      .describe(
        "Exact target folder URN to publish into, e.g. 'urn:adsk.wipprod:fs.folder:co.xxxxx'. " +
    ...
    folder_path: z
      ...
      .optional()
      .describe(
        "Folder path to resolve (and optionally create) instead of an exact id, e.g. " +
    ...
    hub_id: z
      ...
      .optional()
      .describe(
        "Hub id (e.g. 'b.ba36cdef-...'), from list_hubs. Lets the tool skip hub discovery. " +
    ...
    hub_name: z
      ...
      .optional()
      .describe(
        "Hub name to narrow project resolution, e.g. 'Demoland'. Optional — combine with region to " +
    ...
    create_missing: z
      ...
      .default(true)
      .describe(
        "When resolving folder_path: create any missing subfolder segments. Default true. " +
    ...
    source_oss_url: z
      ...
      .optional()
      .describe(
        "oss:// URL of an object already in APS OSS to publish — typically the output of a prior job " +
    ...
    file_path: z
      ...
      .optional()
      .describe(
        "Full path to a local file on this Mac (~/Downloads/, /Users/..., OneDrive paths). " +
          "Provide exactly one of source_oss_url, file_path, or file_url."
      ),
    file_url: z
      .string()
      ...
      .optional()
      .describe(
        "HTTPS URL to fetch and publish (e.g. a public/sharing link). " +
          "Provide exactly one of source_oss_url, file_path, or file_url."
      ),
    file_name: z
      .string()
      .optional()
      .describe(
        "Display name for the file in ACC (e.g. 'tower.ifc'). " +
    ...
    if_exists: z
      ...
      .default("new_version")
      .describe(
        "What to do when a file with this name already exists in the target folder. " +
    ...
    open_in_browser: z
      ...
      .default(true)
      .describe(
        "On success, open the ACC Docs folder (now containing the file) in the default browser. " +
    ...
    region: z
      ...
      .optional()
      .describe(
        "Data-residency region of the hub, sent as x-ads-region on the storage, OSS, and item calls. " +
  ...
  .describe(
    "Publish a file into an ACC/BIM360 project folder in one call: reserves storage, uploads the bytes " +
...
export type PublishToAccFolderInput = z.infer<typeof publishToAccFolderSchema>;

export interface PublishToAccFolderResult {
  ...
  created_new_version?: boolean; // true = file already existed, a new version was added (not a new item)
  web_url?: string;              // ACC Docs deep-link to the folder containing the file (the one opened)
  project_files_url?: string;    // guaranteed-valid fallback: the project's files root
  opened_in_browser?: boolean;   // true = the web_url was launched in the default browser
  // Folder resolution outcome (when folder_path was used)
  folder_resolved_from_path?: string;
  folders_created?: string[]; // names of any subfolders created during resolution ([] = all reused)
  error?: string;
  hint?: string;
}
...
const IFC_LIKE: Record<string, string> = {
...
const ACC_WEB_HOSTS: Record<string, string> = {
  US: "acc.autodesk.com",
...
function accWebHost(region: string | undefined): string {
  return (region && ACC_WEB_HOSTS[region.toUpperCase()]) || ACC_WEB_HOSTS.US;
}

function accFolderUrl(region: string | undefined, projectId: string, folderId: string): string {
...
function accProjectUrl(region: string | undefined, projectId: string): string {
  const projectGuid = projectId.replace(/^b\./, "");
...
function parseStorageUrn(urn: string): { bucketKey: string; objectKey: string } | null {
...
export async function handlePublishToAccFolder(
  input: PublishToAccFolderInput
): Promise<PublishToAccFolderResult> {
  // ── Exactly one source ────────────────────────────────────────────────────
  ...
  if (!cred) {
    return {
      ...
      hint:
        "Run authenticate_aps_3lo first to sign in. A 2LO client-credential token cannot see or write " +
  ...
  try {
    ...
    if (err instanceof ResolveError) return { status: "error", error: err.message, hint: err.hint };
  ...
  if (input.folder_id) {
    ...
    try {
      ...
      if (err instanceof FolderResolveError) return { status: "error", error: err.message, hint: err.hint };
      const e = err as DAError;
  ...
  try {
    ...
    if (err instanceof SourceError) return { status: "error", error: err.message, hint: err.hint };
  ...
  try {
    ...
    if (!signed.urls?.length) throw new DAError("OSS returned no upload URL.");
...
async function acquireBytes(
    ...
    if (!existsSync(p)) {
      ...
      throw new SourceError(
        isOneDrive
          ...
          : `File not found: '${p}'`,
        isOneDrive ? "Click the file once in Finder to sync it locally, then retry." : undefined
      );
    }
    if (!statSync(p).isFile()) throw new SourceError(`Path is not a file: '${p}'`);
  ...
  if (!res.ok) throw new SourceError(`Failed to fetch URL (HTTP ${res.status}): ${fileUrl}`);
...
async function downloadOssObject(ossUrl: string, threeLeggedToken: string): Promise<Buffer> {
  const tokens: string[] = [];
  try {
  ...
  throw new SourceError(
    `Could not download the source object with app (2LO) or user (3LO) credentials: ${String(lastErr)}`,
    "If this is a Design Automation / Model Derivative output, it sits in an app-owned bucket — " +
...
function dmHeaders(token: string, region?: string): Record<string, string> {
  const h: Record<string, string> = {
    Authorization: `Bearer ${token}`,
    "Content-Type": "application/vnd.api+json",
    Accept: "application/vnd.api+json",
  };
  if (region) h["x-ads-region"] = region.toUpperCase();
  return h;
}
...
async function createStorage(
  ...
  if (!res.ok) throw new DAError(await res.text(), res.status);
  const json = (await res.json()) as { data?: { id?: string } };
  if (!json.data?.id) throw new DAError("create_storage response missing data.id");
  return json.data.id;
}
...
async function createItem(
  ...
  if (!json.data?.id) throw new DAError("create_item response missing data.id");
...
function folderName(attrs: Record<string, unknown> | undefined): string {
  if (!attrs) return "";
  return ((attrs.name as string) || (attrs.displayName as string)) ?? "";
}
...
async function listTopFolders(
  ...
  const res = await fetch(
    `${PROJECT_BASE}/hubs/${encodeURIComponent(hubId)}/projects/${encodeURIComponent(projectId)}/topFolders`,
    { headers: dmHeaders(token, region) }
  );
  if (!res.ok) throw new DAError(await res.text(), res.status);
  const json = (await res.json()) as {
  ...
  return (json.data ?? [])
    .filter((d): d is { id: string; attributes?: Record<string, unknown> } => Boolean(d.id))
...
async function listSubfolders(
  ...
  const out: FolderRef[] = [];
  let url: string | null =
    `${DM_BASE}/projects/${encodeURIComponent(projectId)}/folders/${encodeURIComponent(folderId)}/contents` +
    `?filter[type]=folders&page[limit]=200`;
  let guard = 0;
  while (url && guard++ < 25) {
    const res: Response = await fetch(url, { headers: dmHeaders(token, region) });
    if (!res.ok) throw new DAError(await res.text(), res.status);
    const json = (await res.json()) as {
    ...
    for (const d of json.data ?? []) {
      if (d.type === "folders" && d.id) out.push({ id: d.id, name: folderName(d.attributes) });
    }
...
async function createFolder(
  ...
  if (!res.ok) throw new DAError(await res.text(), res.status);
  const json = (await res.json()) as { data?: { id?: string } };
  if (!json.data?.id) throw new DAError("create_folder response missing data.id");
  return json.data.id;
}
...
async function resolveFolderPath(
  ...
  token: string,
  region?: string
): Promise<{ folderId: string; created: string[] }> {
  const segments = rawPath
    .split("/")
    .map((s) => s.trim())
    .filter(Boolean);
  if (segments.length === 0) throw new FolderResolveError("folder_path is empty.");

  const topFolders = await listTopFolders(hubId, projectId, token, region);
  if (topFolders.length === 0) {
    throw new FolderResolveError(
      "No top folders returned for this project.",
      "Check hub_id and project_id match this hub (and pass region for non-US hubs)."
    );
  }
  ...
  if (topMatch) {
    ...
    if (!def) {
      throw new FolderResolveError(
        `'${segments[0]}' is not a top folder, and the default top folder '${DEFAULT_TOP_FOLDER}' was not found.`,
        `Available top folders: ${topFolders.map((f) => f.name).join(", ")}. ` +
  ...
  for (const seg of remaining) {
    ...
    if (matches.length === 1) {
      ...
      throw new FolderResolveError(
        `Ambiguous: ${matches.length} folders named '${seg}' already exist at this level.`,
        `Matching folder ids: ${matches.map((m) => m.id).join(", ")}. Pass the exact one as folder_id.`
      );
      ...
      throw new FolderResolveError(
        `Folder '${seg}' does not exist and create_missing is false.`,
        "Set create_missing: true to create it, or pass an existing folder_id."
      );
    }
  }
...
async function listHubs(token: string): Promise<HubRef[]> {
  ...
  return (json.data ?? [])
    ...
    .map((d) => ({
      ...
      region: String((d.attributes?.region as string) ?? "").toUpperCase(),
    }));
}

async function listHubProjects(hubId: string, token: string): Promise<ProjectRef[]> {
  const out: ProjectRef[] = [];
  let url: string | null = `${PROJECT_BASE}/hubs/${encodeURIComponent(hubId)}/projects?page[limit]=200`;
  let guard = 0;
  while (url && guard++ < 25) {
    const res: Response = await fetch(url, { headers: dmHeaders(token) });
    if (!res.ok) throw new DAError(await res.text(), res.status);
    const json = (await res.json()) as {
      data?: Array<{ id?: string; attributes?: Record<string, unknown> }>;
      links?: { next?: { href?: string } | string };
    ...
    for (const d of json.data ?? []) {
      if (d.id) out.push({ id: d.id, name: (d.attributes?.name as string) ?? "" });
    }
    const next = json.links?.next;
    url = typeof next === "string" ? next : (next?.href ?? null);
  }
...
async function resolveProject(
  q: { projectId?: string; projectName?: string; hubId?: string; hubName?: string; region?: string },
  token: string
): Promise<{ hubId?: string; projectId: string }> {
  if (q.projectId && q.hubId) return { hubId: q.hubId, projectId: q.projectId };
  ...
  if (q.hubId) {
    ...
    if (q.hubName) hubs = hubs.filter((h) => nameEq(h.name, q.hubName));
    if (q.region) hubs = hubs.filter((h) => h.region === q.region!.toUpperCase());
    if (hubs.length === 0) {
      throw new ResolveError(
        "No hubs matched the given hub_name/region.",
        "Drop or correct hub_name/region, or pass hub_id directly."
      );
    }
  }
  ...
  if (q.projectId) {
    const matches: Array<{ hubId: string; projectId: string }> = [];
    for (const h of hubs) {
      const projects = await listHubProjects(h.id, token);
      if (projects.some((p) => p.id === q.projectId)) matches.push({ hubId: h.id, projectId: q.projectId });
    }
    if (matches.length === 1) return matches[0];
    if (matches.length === 0) return { hubId: q.hubId, projectId: q.projectId }; // still usable for folder_id mode
    throw new ResolveError(
      `project_id ${q.projectId} matched in ${matches.length} hubs.`,
      `Pass hub_id to disambiguate. Hubs: ${matches.map((m) => m.hubId).join(", ")}.`
    );
  }

  // Resolve by project_name across candidate hubs.
  const named: Array<{ hubId: string; hubName: string; projectId: string }> = [];
  for (const h of hubs) {
    const projects = await listHubProjects(h.id, token);
    for (const p of projects) {
      if (nameEq(p.name, q.projectName)) named.push({ hubId: h.id, hubName: h.name, projectId: p.id });
    }
  }
  if (named.length === 1) return { hubId: named[0].hubId, projectId: named[0].projectId };
  if (named.length === 0) {
    throw new ResolveError(
      `No project named '${q.projectName}' found` +
        `${q.hubName ? ` in hub '${q.hubName}'` : ""}${q.region ? ` (region ${q.region})` : ""}.`,
      "Check the exact project name (case-sensitive), or pass project_id."
    );
  }
  throw new ResolveError(
    `${named.length} projects named '${q.projectName}' exist across hubs.`,
    `Narrow with hub_name or region. Candidates: ${named.map((m) => `${m.hubName || "?"}/${m.projectId}`).join(", ")}.`
  );
}
...
async function findItemByName(
  ...
  while (url && guard++ < 25) {
    ...
    for (const d of json.data ?? []) {
      if (d.type === "items" && d.id && nameEq(d.attributes?.displayName as string, fileName)) return d.id;
    }
    const next = json.links?.next;
    url = typeof next === "string" ? next : (next?.href ?? null);
  }
...
async function createVersion(
  ...
  if (!res.ok) throw new DAError(await res.text(), res.status);
  const json = (await res.json()) as { data?: { id?: string } };
  if (!json.data?.id) throw new DAError("create_version response missing data.id");
  return json.data.id;
}
