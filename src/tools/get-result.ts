...
export const getResultSchema = z.object({
  ...
  read_content: z
    ...
    .default(false)
    .describe(
      "Override the large-text auto-save behaviour. When true, return the text content inline even if " +
...
export interface GetResultOutput {
  ...
  save_as_skill_offer?: boolean; // true when the final-output next_action includes the save-as-skill nudge
  error?: string;
  hint?: string;
}
...
const SAVE_SKILL_OFFER =
  "Then — if this was a task the user might run again, and it was NOT launched from a saved " +
...
async function getResultImpl(input: GetResultInput): Promise<GetResultOutput> {
  ...
  if (!input.read_content && input.offset_chars === 0 && totalChars > LARGE_TEXT_THRESHOLD) {
    ...
    if (savedTo) {
      ...
      return {
        ...
        content:
          `[Large text file — ${totalChars.toLocaleString()} chars. Auto-saved to: ${savedTo}]\n\n` +
          `Preview (first 500 chars):\n${preview}`,
        saved_to: savedTo,
        has_more: false,
        truncated: false,
