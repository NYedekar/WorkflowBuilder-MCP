...
const VIEWER_REDIRECT_RE =
  /\b(viewer|visuali[sz]e?|open.{0,15}in.{0,10}viewer|web[\s-]?viewer|aps[\s-]?viewer|forge[\s-]?viewer|see\s+the\s+model|open\s+model\s+in|render\s+model|translate\s+(and\s+)?(view|render|open|show)|view\s+(the\s+)?(rvt|dwg|ifc|model)\b)\b/i;
