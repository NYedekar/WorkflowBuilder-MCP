...
import {
  ...
  collectStepPlaceholders,
  findSecrets,
  slugify,
  type WorkflowManifest,
} from "../lib/skill-manifest.js";
import { renderSkillMd } from "../lib/skill-codegen.js";
import { findCapabilityById, type CapabilityRecord, type OperationRecord } from "../lib/registry-client.js";
...
export const saveWorkflowAsSkillSchema = z.object({
  ...
  description: z
    ...
    .optional()
    .describe(
      "Skill trigger description for the frontmatter — what it does AND when to invoke it (include trigger phrases). " +
...
export type SaveWorkflowAsSkillInput = z.infer<typeof saveWorkflowAsSkillSchema>;

export type SaveWorkflowAsSkillResult =
  | {
      status: "success";
      slug: string;
      skill_dir: string;
  ...
  | { status: "error"; error: string; hint?: string; details?: unknown };
...
function resolveOperation(cap: CapabilityRecord, operationId: string): OperationRecord | undefined {
  const q = operationId.toLowerCase();
  return cap.operations.find(
    (o) => o.operationId.toLowerCase() === q || (o.globalOperationId ?? "").toLowerCase() === q
  );
}

/** Derive 2LO vs 3LO for a single resolved operation. */
function deriveAuth(op: OperationRecord, cap: CapabilityRecord): "2LO" | "3LO" {
  const strategy = op.authStrategy ?? cap.authStrategy;
  if (strategy === "3LO") return "3LO";
  if (strategy === "2LO" || strategy === "either") return "2LO";
  const flows = (op.authFlows ?? cap.authFlows ?? []).map((f) => f.toLowerCase());
  if (flows.some((f) => f.includes("authorization_code") || f.includes("3lo") || f.includes("pkce"))) return "3LO";
  return "2LO";
}

function defaultDescription(name: string, intent: string): string {
  return (
    `${intent} ` +
...
export async function handleSaveWorkflowAsSkill(
  input: SaveWorkflowAsSkillInput
): Promise<SaveWorkflowAsSkillResult> {
  const warnings: string[] = [];
  const slug = slugify(input.name);

  // ── 1. Validate every step against the registry; derive auth ──────────────
  let derivedAuth: "2LO" | "3LO" = "2LO";
  for (const step of input.steps) {
    ...
    if (deriveAuth(op, cap) === "3LO") derivedAuth = "3LO";
  }

  const authMode = input.auth_mode === "auto" ? derivedAuth : input.auth_mode;
  if (input.auth_mode !== "auto" && input.auth_mode !== derivedAuth) {
    warnings.push(
      `auth_mode was set to '${input.auth_mode}' but the resolved operations imply '${derivedAuth}'. Using '${input.auth_mode}' as requested.`
    );
  }
  ...
  if (unused.length) warnings.push(`Declared input(s) never referenced by any step: ${unused.join(", ")}.`);
  ...
  return {
    ...
    warnings,
    next_action:
      `Saved. Tell the user they can now run this workflow anytime with /${slug} (a different input each time, same execution). ` +
