...
export type ExportSkillForClaudeInput = z.infer<typeof exportSkillForClaudeSchema>;

export type ExportSkillForClaudeResult =
  | {
      status: "success";
      slug: string;
      zip_path: string;
...
export async function handleExportSkillForClaude(
  input: ExportSkillForClaudeInput
): Promise<ExportSkillForClaudeResult> {
  // Validate the skill exists and is a real saved workflow.
  const loaded = loadManifest(input.slug, input.skills_dir);
  if (!loaded.ok) return { status: "error", error: loaded.error, hint: loaded.hint };
  ...
  return {
    ...
    caveat:
      "The skill drives this MCP server's tools (get_capability, execute_workflow, run_saved_workflow). " +
    ...
    next_action:
      `ZIP ready at ${zipPath}. There is no API to auto-upload personal skills to claude.ai — ` +
