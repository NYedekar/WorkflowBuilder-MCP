...
export const applyViewerUpdatesSchema = z.object({
  session_id: z
    ...
    .optional()
    .describe(
      "Session ID shown in the viewer's 'Changes Saved' dialog. " +
  ...
  output_rvt_oss_url: z
    ...
    .optional()
    .describe(
      "Provide this ONLY on the second call, after get_workflow_status reports success. " +
...
export type ApplyViewerUpdatesInput = z.infer<typeof applyViewerUpdatesSchema>;

// ── Handler ───────────────────────────────────────────────────────────────

export async function handleApplyViewerUpdates(
  input: ApplyViewerUpdatesInput
  ...
  if (input.output_rvt_oss_url) {
    ...
    try {
      ...
      if (renderResult.status === "success") {
        const filePath = (renderResult as { file_path?: string }).file_path;
        ...
        return {
          ...
          message:
            `Model updated! ${pending.changes.length} parameter change(s) applied. ` +
      ...
      if (renderResult.status === "pending") {
        ...
        return {
          ...
          message: (renderResult as { message?: string }).message ?? "Translation in progress.",
          next_action:
            `SVF2 translation is still running. Wait ~60 seconds then call ` +
      ...
      const errMsg = (renderResult as { error?: string }).error ?? "render_model returned an error";
  ...
  return {
    ...
    next_action:
      "1. Poll get_workflow_status(workflow_handle) until overall_status=success. " +
