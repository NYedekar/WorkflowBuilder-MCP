...
import { runWorkflow, type RunnerDeps, type RunState, type RunResult } from "../lib/workflow-runner.js";
...
export type RunSavedWorkflowInput = z.infer<typeof runSavedWorkflowSchema>;
...
export async function handleRunSavedWorkflow(input: RunSavedWorkflowInput): Promise<RunResult> {
  ...
  return runWorkflow(
    loaded.manifest,
    input.inputs ?? {},
    input.run_handle as RunState | undefined,
    deps
  );
}
