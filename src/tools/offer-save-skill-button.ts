...
export type OfferSaveSkillButtonInput = z.infer<typeof offerSaveSkillButtonSchema>;

// Returns a CallToolResult with structuredContent (forwarded to the linked UI iframe).
export function handleOfferSaveSkillButton(input: OfferSaveSkillButtonInput) {
  const recipe = {
    name: input.name,
    intent: input.intent,
    inputs: input.inputs,
    ...
    content: [
      {
        type: "text" as const,
        text:
          `Want to save this as a reusable skill — "${input.name}" (${input.steps.length} step(s))? ` +
