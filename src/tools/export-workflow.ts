...
export const exportWorkflowSchema = z.object({
  workflow: z
    ...
    .describe(
      "The WorkflowDAG object (or its JSON string) to export"
    ),
  format: z
    .enum(["json", "yaml"])
  ...
  output_path: z
    ...
    .optional()
    .describe(
      "Optional file path to write the output to. If omitted, returns the serialized string only."
    ),
});
  ...
  if (typeof input.workflow === "string") {
    try {
      ...
      return JSON.stringify(
        { error: `Failed to parse workflow JSON string: ${String(err)}` },
        null,
        2
      );
    }
