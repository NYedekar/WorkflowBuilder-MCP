...
export const processFileSchema = z.object({
  ...
  intent: z
    ...
    .optional()
    .describe(
      "What to do with the file — used to auto-select the best capability. " +
  ...
  max_result_chars: z
    ...
    .default(50_000)
    .describe(
      "Max chars of the first chunk of each output file to return. Default and max: 50 000. " +
  ...
  body: z
    ...
    .optional()
    .describe(
      "Extra request body fields merged on top of any defaultBody in the capability registry. " +
...
export interface ProcessFileOutput {
  ...
  // no_capability_found
  gap_note?: string;
  // success / pending common
  capability_used?: string;
  operation_used?: string;
...
export async function handleProcessFile(input: ProcessFileInput): Promise<ProcessFileOutput> {
  // ── Step 1: Upload ────────────────────────────────────────────────────
  ...
  if (!capabilityId || !operationId) {
    ...
    if (!capabilityId) {
      ...
      let foundCap: ReturnType<typeof searchCapabilities>[number] | undefined;
      ...
      for (const q of queries) {
        ...
        if (callable) { foundCap = callable; usedQuery = q; break; }
      }
      ...
      if (!foundCap) {
        return {
          ...
          gap_note:
            `⚠️ WorkflowSkills gap — searched: ${queries.map(q => `"${q}"`).join(", ")} | ` +
      ...
      if (!operationId) {
        ...
        if (op) operationId = op.operationId;
      }
  ...
  let effectiveBody: Record<string, unknown> | undefined;
