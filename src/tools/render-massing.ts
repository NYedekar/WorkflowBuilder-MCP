...
interface MassingData {
  name: string;
  footprint: { w: number; d: number }; // plan extent in model units (from grid bounds)
  levels: { name: string; elevation: number; height: number }[]; // sorted by elevation
...
export async function handleRenderMassing(input: RenderMassingInput): Promise<RenderMassingOutput> {
  ...
  let levels: { name: string; elevation: number; height: number }[] = [];
  ...
  try {
    ...
    let aecUrn: string | null = null;
    const walk = (ds: any[]) =>
      (ds || []).forEach((d) => {
        if (typeof d.urn === "string" && /AECModelData\.json$/i.test(d.urn)) aecUrn = d.urn;
        if (d.children) walk(d.children);
      });
    walk(manJson.derivatives);
    if (!aecUrn) aecUrn = `urn:adsk.viewing:fs.file:${urn}/output/Resource/AECModelData.json`;

    const aecRes = await apiFetch(`${MD_BASE}/designdata/${urn}/manifest/${encodeURIComponent(aecUrn)}`, token, 45_000);
    if (aecRes.ok) {
      const aec: any = await aecRes.json();
      levels = (aec.levels ?? [])
        .filter((l: any) => typeof l.elevation === "number")
        .map((l: any) => ({ name: String(l.name ?? "Level"), elevation: l.elevation, height: Number(l.height) || 0 }))
      ...
      for (const g of aec.grids ?? []) {
        ...
        if (Array.isArray(bb) && bb.length >= 6) {
          minX = Math.min(minX, bb[0], bb[3]); maxX = Math.max(maxX, bb[0], bb[3]);
          minY = Math.min(minY, bb[1], bb[4]); maxY = Math.max(maxY, bb[1], bb[4]);
        }
      }
  ...
  try {
    ...
    if (guid) {
      ...
      const catNodes: any[] = root?.objects ?? [];
      categories = catNodes
  ...
  const massing: MassingData = { name: modelName, footprint, levels, categories, totalElements };
  ...
  return {
    ...
    massing,
    message:
      `Massing extracted (${levels.length} levels, footprint ${footprint.w.toFixed(0)}×${footprint.d.toFixed(0)}, ` +
