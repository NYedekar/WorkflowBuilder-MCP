...
// re-uploaded within a session. TTL is 30 min — well within transient bucket
// 24h expiry. Cache is in-process only; restarts start fresh.

interface OssCacheEntry { ossUrl: string; cachedAt: number; }
const ossUploadCache = new Map<string, OssCacheEntry>();
const CACHE_TTL_MS = 30 * 60 * 1000;

function getCachedOssUrl(key: string): string | null {
  const entry = ossUploadCache.get(key);
  if (!entry) return null;
  if (Date.now() - entry.cachedAt > CACHE_TTL_MS) { ossUploadCache.delete(key); return null; }
  return entry.ossUrl;
}

function setCachedOssUrl(key: string, ossUrl: string): void {
...
export const uploadFileSchema = z.object({
  file_path: z
    ...
    .optional()
    .describe(
      "Full path to the file on this Mac — ~/Downloads/, /Users/..., or OneDrive paths. " +
  ...
  file_url: z
    ...
    .optional()
    .describe(
      "HTTPS URL to fetch and upload directly to APS OSS. " +
...
export async function handleUploadFile(input: UploadFileInput): Promise<UploadFileResult> {
  ...
  if (cachedUrl) {
    ...
    if (resolvedPath.toLowerCase().endsWith(".zip") && !getZipPathHint(cachedUrl)) {
      ...
      if (rootIam) setZipPathHint(cachedUrl, rootIam);
    }
  ...
  if (resolvedPath.toLowerCase().endsWith(".zip")) {
    const rootIam = findRootIamInZip(resolvedPath);
    if (rootIam) setZipPathHint(ossUrl, rootIam);
  }
...
async function handleUrlUpload(input: UploadFileInput): Promise<UploadFileResult> {
  ...
  try {
    signedUpload = await getSignedS3UploadUrl(
      cred.access_token, bucketKey, objectKey,
      input.signed_url_expiry_minutes ?? 60, numParts
    );
  } catch (err) {
    return { status: "error", error: `Could not get upload URL: ${String(err)}` };
  }
