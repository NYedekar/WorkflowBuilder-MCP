...
import { workflowManifestSchema, type WorkflowManifest } from "./skill-manifest.js";
...
export type LoadManifestResult =
  | { ok: true; manifest: WorkflowManifest; dir: string }
  | { ok: false; error: string; hint?: string };

/** Load and validate a single saved workflow by slug. */
export function loadManifest(slug: string, skillsDir?: string): LoadManifestResult {
...
export function listManifests(skillsDir?: string): ManifestSummary[] {
  ...
  for (const entry of readdirSync(root)) {
    ...
    if (!res.ok) continue; // skip non-workflow skills and invalid manifests
