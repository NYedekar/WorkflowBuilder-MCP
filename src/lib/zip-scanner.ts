...
function readCentralDirectory(buf: Buffer): ZipEntry[] {
  ...
  while (pos + 46 <= cdOffset + cdSize && pos + 46 <= buf.length) {
    ...
    if (pos + 46 + fnLen > buf.length) break;
 ...
 */
export function findRootIamInZip(zipPath: string): string | null {
  let buf: Buffer;
  try {
  ...
  const iamEntries = entries.filter(
    (e) =>
      e.name.toLowerCase().endsWith(".iam") &&
      !e.name.startsWith("__MACOSX/")
  );

  if (iamEntries.length === 0) return null;
  if (iamEntries.length === 1) return iamEntries[0].name;
