...
import {
  publishToAccFolderSchema,
  handlePublishToAccFolder,
} from "./tools/publish-to-acc-folder.js";
import {
  getResultSchema,
  handleGetResult,
...
import {
  offerSaveSkillButtonSchema,
  handleOfferSaveSkillButton,
} from "./tools/offer-save-skill-button.js";
...
import {
  extractBimDataSchema,
  handleExtractBimData,
} from "./tools/extract-bim-data.js";
import {
  pushToBimDashboardSchema,
  handlePushToBimDashboard,
} from "./tools/push-to-bim-dashboard.js";
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          "Obtain a user-identity (3LO) token for ACC admin operations that reject 2LO client-credential tokens. " +
      ...
      },
      {
        name: "get_capability",
        description:
          "CALL THIS FIRST when any Autodesk file extension or cloud service appears. " +
          ...
          "⛔ RENDER/VIEW EXCEPTION: if the intent is to RENDER, VIEW, VISUALIZE, or OPEN a model in a viewer, " +
          "do NOT call get_capability — call render_model directly instead. " +
          "render_model is the complete end-to-end tool (upload if needed → translate → open HTML viewer in browser). " +
          "Manually assembling translate+manifest+viewer URL via get_capability/process_file/execute_workflow will NOT work " +
          "— there is no public APS Viewer URL to construct; the HTML must be generated by render_model.",
        inputSchema: zodToJsonSchema(getCapabilitySchema),
      },
      {
        name: "execute_workflow",
        description:
          "PREREQUISITE: capability_id MUST come from a get_capability() call in this session — NEVER use an ID from memory or training data. Unregistered IDs fail immediately. " +
      ...
      },
      {
        name: "process_file",
        description:
          "BEFORE CALLING: count how many intents target this exact file_path. " +
      ...
      },
      {
        name: "upload_file",
        description:
          "PREREQUISITE: get_capability must have been called for all intents, and the execution plan (Step 3) presented, before calling this. " +
      ...
      },
      {
        ...
        description:
          "Publish a file into an ACC/BIM360 project folder in ONE call — the supported alternative to " +
      ...
      },
      {
        ...
        description:
          "Poll the status of one or more running Autodesk DA jobs. CALL THIS AUTOMATICALLY — no user confirmation needed. " +
      ...
      },
      {
        name: "render_model",
        description:
          "THE complete end-to-end tool for viewing any APS/Autodesk model (RVT, DWG, IFC, etc.) in the browser. " +
          "DO NOT use process_file + execute_workflow + manual URL construction — that path produces a fake URL that does not work. " +
          "This tool is the ONLY way to get a working viewer: it handles SVF2 translation, builds a self-contained " +
          "HTML file, saves it to ~/Downloads, and AUTO-OPENS it in the browser. " +
          "mode='viewer' (default): translates to SVF2, saves interactive Autodesk Viewer HTML to ~/Downloads, auto-opens in browser (returns file_path). " +
          "The .html file is EMAILABLE — valid ~1h until the embedded token expires. " +
          "If status=pending (translating), re-call until success — do NOT construct a viewer URL yourself. " +
          "If stuck at 99%, re-call with force_retranslate=true to clear and restart. " +
          "Like get_result, pass model + estimated_input_tokens + estimated_output_tokens to auto-record token usage. " +
      ...
      },
      {
        ...
        description:
          "Query the token-usage log written by record_token_usage. " +
      ...
      },
      {
        ...
        description:
          "Freeze a workflow you JUST ran into a reusable Claude skill the user can invoke as /<slug> with different inputs each time. " +
      ...
      },
      {
        ...
        description:
          "EXPERIMENTAL (MCP Apps): render an interactive 'Save as skill' button in the conversation. " +
      ...
      },
      {
        ...
        description:
          "Push approved BIM element data to Supabase and update the Lovable live dashboard in real-time. " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    const responsePayload = rateLimitWarning && result && typeof result === "object"
      ? { ...result as object, _rate_limit_warning: rateLimitWarning }
      : result;
