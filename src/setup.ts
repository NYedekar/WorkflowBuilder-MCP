...
function updateMcpJson(clientId: string, clientSecret?: string): {
  success: boolean;
  path: string;
  error?: string;
} {
  try {
    ...
    try {
      config = JSON.parse(readFileSync(CLAUDE_JSON_PATH, "utf8")) as ClaudeJson;
    } catch {
...
function loadFromFile(filePath: string): { clientId: string; clientSecret: string } {
  let raw: string;
  try {
  ...
  let parsed: CredFile;
  try {
    parsed = JSON.parse(jsonText) as CredFile;
  } catch {
...
async function main(): Promise<void> {
  ...
  if (result.success) {
    ...
    print("");
    const envBlock: Record<string, string> = { APS_CLIENT_ID: clientId };
    if (secretInFile) envBlock["APS_CLIENT_SECRET"] = clientSecret;
