...
export function buildPromptList(skillsDir) {
    const prompts = [];
...
export function buildPromptMessages(name, args, skillsDir) {
    ...
    const lines = m.inputs.map((i) => {
        ...
        if (i.required && i.default === undefined)
            return `- ${i.key}: (MISSING — ask me for this before running)`;
    ...
    const text = `Run my saved workflow "${m.name}" (${m.intent}).\n\n` +
