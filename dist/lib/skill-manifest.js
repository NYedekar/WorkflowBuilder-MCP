...
export const AUTH_MODES = ["2LO", "3LO", "auto"];
...
export function collectPlaceholders(value, found = new Set()) {
...
export function collectStepPlaceholders(steps) {
    const found = new Set();
...
export function findSecrets(steps) {
    const hits = [];
    for (const s of steps) {
        const scan = (obj, base) => {
            if (!obj)
            ...
            for (const k of Object.keys(obj)) {
                if (SECRET_KEY_RE.test(k))
                    hits.push({ stepId: s.id, location: `${base}.${k}` });
            }
        };
        scan(s.args, "args");
