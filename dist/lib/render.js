...
function edgeLabel(edge) {
    if (!edge)
        return "(parallel branch — not a direct edge; see parallel groups below)";
    const base = {
...
export function renderDagAscii(dag) {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        if (!node)
            continue;
        for (const l of renderBox(i + 1, node))
            lines.push(l);
        if (i < order.length - 1) {
    ...
    lines.push("");
    lines.push(`nodes: ${dag.metadata.intent_count}  ` +
