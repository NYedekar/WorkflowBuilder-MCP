...
export const uploadFileSchema = z.object({
    ...
    region: z
        .string()
        .optional()
        .describe("Data-residency region for the OSS bucket, sent as x-ads-region on bucket creation. " +
...
async function ensureBucketWithPolicy(token, bucketKey, policy, region) {
        ...
        if (contentTypeHdr.includes("text/html")) {
            ...
            if (!retryRes.ok || (retryRes.headers.get("content-type") ?? "").includes("text/html")) {
                return {
                    ...
                    hint: `Try appending '?download=1' to the URL, or use "Copy direct link" / "Download" in OneDrive ` +
