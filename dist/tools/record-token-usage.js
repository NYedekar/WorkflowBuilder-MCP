...
export const recordTokenUsageSchema = z.object({
    model: z
        .string()
        .describe("The model ID that generated these tokens (e.g. 'claude-sonnet-4-6', 'claude-opus-4-8'). " +
    ...
    workflow_id: z
        .string()
        .optional()
        .describe("Workflow DAG ID if this usage was incurred while building or executing a create_workflow DAG. " +
        "Match the 'id' field returned by create_workflow."),
    capability_id: z
        .string()
        .optional()
        .describe("Capability ID that was executed (e.g. 'RevitElementDataExtraction'). " +
    ...
    session_id: z
        .string()
        .optional()
        .describe("Optional session identifier to group multiple calls in the same conversation. " +
...
export async function handleRecordTokenUsage(input) {
    const record = {
