...
export const renderModelSchema = z.object({
    ...
    root_filename: z
        .string()
        .optional()
        .describe("Override the filename hint sent to Model Derivative. Use when the OSS object key has the wrong " +
...
async function startSvf2Translation(token, urn, region, force, rootFilename) {
    const headers = {
    ...
    const inputBlock = { urn };
    if (rootFilename)
        inputBlock.rootFilename = rootFilename;
...
function buildViewerHtml(urn, token, tokenTtlSeconds, sessionId, ossUrl) {
...
export async function handleRenderModel(input) {
    ...
    if (!manifest || input.force_retranslate) {
        ...
        return {
            status: "pending",
            urn,
            message: `${restartNote}SVF2 translation started. Call render_model again in 30–60 seconds to check progress. ` +
    ...
    if (manifest.status !== "success") {
        ...
        if (isStuck99) {
            return {
                status: "pending",
                urn,
                message: `Translation is stuck at ${progressStr} — this is a known APS stall. ` +
        ...
        return {
            status: "pending",
            urn,
            message: `Translation ${manifest.status} (${progressStr}). ` +
