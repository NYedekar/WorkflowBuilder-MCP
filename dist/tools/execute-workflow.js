...
export const executeWorkflowSchema = z.object({
    capability_id: z
        .string()
        .describe("Capability ID or alias from get_capability. " +
    ...
    input_file_url: z
        .string()
        .optional()
        .describe("Input file URL for Engine-API or REST workflows. " +
    ...
    poll_timeout_ms: z
        ...
        .optional()
        .default(50_000)
        .describe("Max milliseconds to poll for Engine-API WorkItem completion per call. " +
...
export async function handleExecuteWorkflow(input) {
    ...
    const cap = findCapabilityById(input.capability_id) ??
    ...
    // 2. Resolve operation
    const op = cap.operations.find((o) => o.operationId === input.operation_id ||
...
async function executeRest(cap, op, input, t0) {
    ...
    if (input.input_file_url?.startsWith("oss://")) {
        ...
        const b64urn = Buffer.from(urnRaw)
            ...
            .replace(/\//g, "_")
            .replace(/=+$/, "");
        const existingInput = (effectiveBody.input ?? {});
    ...
    if (op.asyncJob) {
        ...
        result.async_job_note = buildPollingNote(op, responseBody);
    }
    return result;
}
...
function resolveScopes(authScopes, fallback) {
...
function httpHint(status) {
    ...
    if (status === 422)
        return "Unprocessable entity — check the body payload matches the expected schema.";
    if (status === 429)
        return "Rate limited. Retry after a short wait.";
    if (status >= 500)
        return "APS server error. Check https://health.autodesk.com or retry.";
    return "";
}
function deriveInputArgName(ioShape) {
    const lower = ioShape.toLowerCase();
    if (lower.includes(".rvt"))
        return "rvtFile";
    if (lower.includes(".dwg"))
        return "dwgFile";
    if (lower.includes(".ipt") || lower.includes(".iam"))
        return "inventorFile";
    if (lower.includes(".f3d") || lower.includes("fusion"))
        return "f3dFile";
    if (lower.includes(".max") || lower.includes("3dsmax"))
        return "maxFile";
    return "inputFile";
}
...
function outputContentType(filename) {
...
function buildPollingNote(op, jobResponse) {
    ...
    for (const [paramName, responsePath] of Object.entries(path_param_map)) {
        const value = responsePath.split(".").reduce((obj, key) => obj?.[key], jobResponse);
        resolvedParams[paramName] = value != null ? String(value) : `<${responsePath}>`;
    }
    const paramsStr = Object.entries(resolvedParams)
        .map(([k, v]) => `${k}: '${v}'`)
        .join(", ");
    return (`Async job accepted. Poll for completion with:\n` +
...
function safeEncodeParam(value) {
    try {
...
function resolveEngineAlias(engineField) {
    const lower = engineField.toLowerCase();
    if (lower.includes("revit"))
        return "Autodesk.Revit+2025";
    if (lower.includes("autocad"))
        return "Autodesk.AutoCAD+25";
    if (lower.includes("fusion"))
        return "Autodesk.Fusion+latest";
    if (lower.includes("inventor"))
        return "Autodesk.Inventor+2025";
    if (lower.includes("3dsmax") || lower.includes("3ds max"))
        return "Autodesk.3dsMax+2025";
    return engineField;
}
