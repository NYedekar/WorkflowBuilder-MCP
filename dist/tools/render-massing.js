...
function findElevation(props) {
    if (!props || typeof props !== "object")
        return null;
    for (const [k, v] of Object.entries(props)) {
        if (/^elevation$/i.test(k) && typeof v === "number")
            return v;
...
export async function handleRenderMassing(input) {
    ...
    try {
        ...
        const roots = treeJson?.data?.objects ?? [];
        const root = roots[0];
        if (root?.name)
            modelName = root.name;
        const catNodes = root?.objects ?? roots;
        categories = catNodes
    ...
    const massing = { name: modelName, levels, categories, totalElements };
    ...
    return {
        ...
        massing,
        message: `Massing extracted (${levels.length} levels, ${categories.length} categories, ${totalElements} elements). ` +
