...
export const authenticateApsSchema = z.object({
    store_to_keychain: z
        ...
        .optional()
        .default(true)
        .describe("Store the client secret to the OS keychain so future MCP restarts " +
        "don't need APS_CLIENT_SECRET in the env block. Default: true."),
    scopes: z
        .array(z.string())
        .optional()
        .describe("APS OAuth scopes to request. Defaults to: data:read, data:write, " +
        "data:create, bucket:create, bucket:read, bucket:update."),
});
export async function handleAuthenticateAps(input) {
    const clientId = process.env.APS_CLIENT_ID?.trim();
    if (!clientId) {
        return {
            status: "error",
            error: "Missing environment variable: APS_CLIENT_ID",
            hint: "Run the setup script to configure credentials:\n" +
    ...
    if (!clientSecret) {
        return {
            ...
            hint: "Run the setup script to store your credentials:\n" +
                "  cd <mcp-workflow-builder dir> && npm run setup\n" +
