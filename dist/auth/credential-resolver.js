...
export async function resolve3LOCredential(scopes = DEFAULT_3LO_SCOPES) {
    const clientId = process.env.APS_CLIENT_ID?.trim();
    if (!clientId)
        return null;
...
export function set3LOToken(clientId, accessToken, expiresIn, scopes = DEFAULT_3LO_SCOPES) {
    const cacheKey = `3lo:${clientId}:${scopes.slice().sort().join(",")}`;
